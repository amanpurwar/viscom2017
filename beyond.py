...
##before running modiy the directory 
...
tmp_high=[i for (s,i) in Temper if s=='high']
lx_high=[lx[i] for i in range(len(Temper)) if Temper[i][0]=='high']
tmp_med=[ i for (s,i) in Temper if s=='medium']
lx_med=[lx[i] for i in range(len(Temper)) if Temper[i][0]=='medium']
tmp_low=[i for (s,i) in Temper if s=='low']
lx_low=[lx[i] for i in range(len(Temper)) if Temper[i][0]=='low']

print(tmp_med)

high=plt.scatter(lx_high,tmp_high,marker='x',color='r')
med=plt.scatter(lx_med,tmp_med,marker='o',color='b')
low=plt.scatter(lx_low,tmp_low,marker='*',color='g')
...
med=plt.scatter(lx_med,tmp_med,marker='o',color='b')
low=plt.scatter(lx_low,tmp_low,marker='*',color='g')
plt.legend((high,med,low),('High','Medium','Low'),scatterpoints=1,loc='lower left',ncol=3)
plt.suptitle("Temper")
plt.scatter(lx,temo_value,color=temo_color)
plt.axis([0,tmp+10000,0,100.0])

tmp_pos=[i for (s,i) in Valence if s=='positive']
print(tmp_pos)

lx_pos=[lx[i] for i in range(len(Valence)) if Valence[i][0]=='positive']
tmp_neut=[i for (s,i) in Valence if s=='neutral']
lx_neut=[lx[i] for i in range(len(Valence)) if Valence[i][0]=='neutral']
tmp_neg=[i for (s,i) in Valence if s=='negative']
lx_neg=[lx[i] for i in range(len(Valence)) if Valence[i][0]=='negative']
...
pos=plt.scatter(lx_pos,tmp_pos,marker='x',color='r')
neut=plt.scatter(lx_neut,tmp_neut,marker='o',color='b')
neg=plt.scatter(lx_neg,tmp_neg,marker='*',color='g')
plt.legend((pos,neut,neg),('Positive','Neutral','Negative'),scatterpoints=1,loc='lower left',ncol=3)
#plt.scatter(lx,vemo_value,color=vemo_color)
plt.axis([0,tmp+10000,0,100.0])



tmp_high=[i for (s,i) in Arousal if s=='high']
lx_high=[lx[i] for i in range(len(Arousal)) if Arousal[i][0]=='high']
tmp_low=[i for (s,i) in Arousal if s=='low']
lx_low=[lx[i] for i in range(len(Arousal)) if Arousal[i][0]=='low']
...
high=plt.scatter(lx_high,tmp_high,marker='x',color='r')
low=plt.scatter(lx_low,tmp_low,marker='*',color='g')
plt.legend((high,low),('High','Low'),scatterpoints=1,loc='lower right',ncol=2)
plt.axis([0,tmp+10000,0,100.0])
plt.show()
